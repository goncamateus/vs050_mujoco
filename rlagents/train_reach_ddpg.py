"""DDPG training for VS050 ReachPose environment.

CleanRL-style single-file implementation.
Trains a deterministic actor-critic with target networks and replay buffer.

Usage:
    uv run --group cleanrl python -m examples.train_reach_ddpg --env-id VS050-ReachPose-v0
"""
...
def parse_args():
    ...
    p.add_argument("--learning-starts", type=int, default=2500)
...
def evaluate(env, actor, device, n_episodes: int):
    """Run deterministic evaluation (no action noise)."""
    actor.eval()
    returns = []
    with torch.no_grad():
