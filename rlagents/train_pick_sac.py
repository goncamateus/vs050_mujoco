"""SAC training for VS050 PickAndPlace environment.

CleanRL-style single-file implementation.
Trains a stochastic actor with dual Q-networks and entropy scheduling.

Usage:
    uv run --group cleanrl python -m examples.train_pick_sac --env-id VS050-PickAndPlace-v0
"""
...
def main():
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step >= args.learning_starts:
            ...
            last_actor_loss = (-min_q_out.mean()).item()
            (-min_q_out).backward()
            actor_optim.step()
            ...
            if args.auto_alpha and log_alpha is not None and alpha_optim is not None:
                with torch.no_grad():
