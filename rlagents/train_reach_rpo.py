"""RPO training for VS050 ReachPose environment.
...
Uses PPO's GAE advantage estimation + projected gradient ascent on
the actor loss to find worst-case action perturbations (PGD-style).

Usage:
    uv run --group cleanrl python -m examples.train_reach_rpo --env-id VS050-ReachPose-v0
"""
    ...
    for epoch in range(args.total_timesteps // storage_size):
        ...
        for mini_epoch in range(args.updates_per_step):
            for start in range(0, storage_size, args.mini_batch_size):
                ...
                # Final actor loss with adversarial perturbation
                perturbed_act = torch.clamp(batch_act + adv_noise.data, -1.0, 1.0)
                final_dist = torch.distributions.Normal(mean, std)
                final_logprob = final_dist.log_prob(perturbed_act).sum(dim=-1)
